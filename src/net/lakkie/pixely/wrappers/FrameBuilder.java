...
public class FrameBuilder
{
	...
	/**
	 * 
	 * This will use reflection to create a frame. For titles, it will search for
	 ...
	 * </ol>
	 * 
	 * When this function tries to set the size, it will use the following methods
	 * to do so(in order):
	 * <ol>
	...
	public static Object instantiateUnsafe(String title, int width, int height, Class<?> type) throws Exception
	{
		...
		if (frame.getClass() != type) { throw new ReflectionCastException("Unable to create instance of " + type.getName()); }
	...
	/**
	 * 
	 * Currently only appends the canvas. Checks for the following functions to be
	 * able to append it:
	 * <ol>
		...
		if (!methodExists(frame.getClass(), "add", Component.class))
		{
			...
			if (!methodExists(frame.getClass(), "add", Canvas.class))
			{
				...
				if (!methodExists(frame.getClass(), "setCanvas", Canvas.class))
				{
					...
					// throw an exception
					throw new BuilderException("Unable to find method to add canvas to frame for " + frame.getClass().getName());
				} else
				{
	...
	/**
	 * 
	 * Sets a frame visible or hidden. Checks for a <code>setVisible(boolean)</code>
	 * method, then <code>show()</code> or <code>hide()</code>
	 * 
	 * @param frame
	 * @param context
	 */
	...
	/**
	 * 
	 * A much more safe option for creating windows, based off of interfaces instead
	 * of reflection. <br>
	 * <b>WARN: </b>This function still uses reflection to create the window, and as
	 * such is not entirely safe.
	 * 
	 * @param <T>
	 ...
	 */
	public static <T extends IAbstractWindow> T instantiateSafe(String title, int width, int height, Class<T> type) throws Exception
	{
